...
public class Entity {
	...
	public boolean update(List<Entity> entities, double deltaTime) {
		double speed = 0.3 * deltaTime;

		if (this.isZombie) {
			if (this.findClosestNonzombie(entities) != null) {
				moveToward(findClosestNonzombie(entities), speed);
				return true;
			} else {
				return false;
			}
		} else {
			moveAwayFrom(findClosestEntity(entities), speed);
			if (this.x >= 1) {
				this.x = 0.99;
			}
			if (this.x <= 0) {
			...
			if (this.isTouching(findClosestZombie(entities))) {
				if (Math.random() < 0.8) {
					...
					if (radius <= 0.02) {
						this.findClosestZombie(entities).radius *= 1.2;
					}
