        ...
        <div className="item-content">
          ...
          {enhancedItem.customTags && enhancedItem.customTags.length > 0 && (
            <div className="item-tags">
              {enhancedItem.customTags.slice(0, 3).map((tag, index) => (
                <span key={index} className="item-tag">
                  #{tag}
                </span>
              ))}
              {enhancedItem.customTags.length > 3 && (
                <span className="item-tag-more">+{enhancedItem.customTags.length - 3}</span>
              )}
            </div>
          )}

          {/* Dates, Rewatch Info, and Position */}
          ...
          {enhancedItem.notes && <p className="item-notes">{enhancedItem.notes}</p>}

          <div className="item-date">Added {new Date(enhancedItem.addedAt).toLocaleDateString()}</div>
        </div>

        {/* Action Buttons */}
        <div className="item-actions">
          {onEdit && (
            <button onClick={() => onEdit(item)} className="action-btn edit" title="Edit item notes">
              <svg fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path
                  strokeLinecap="round"
          ...
          {onRemove && (
            <button onClick={() => onRemove(item.id)} className="action-btn remove" title="Remove from list">
              <svg fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path
                  strokeLinecap="round"
                  strokeLinejoin="round"
